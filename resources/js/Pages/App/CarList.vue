...
const pesanMobil = () => {
    formPesan.post(route('car.order'), {
        onSuccess: (page) => {
            ...
            console.log("POST SUCCESS — should redirect")
        },
        onError: (e) => {
...
<div class="text-center mt-10">
    <h1 class="text-3xl font-bold text-blue-700">Daftar Mobil</h1>
    <p class="text-gray-500 mt-2">Pilih mobil yang tersedia untuk disewa</p>
</div>
...
<div class="flex justify-center items-center gap-2 mt-6">
    <div class="relative w-64">
        ...
        <input
            ...
            class="w-full border rounded-lg pl-8 pr-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        />
    </div>

    <button
        @click="resetFilter"
        class="flex items-center gap-1 px-3 py-2 text-sm bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg transition"
        ...
        >
            ...
            >
                ...
                <div class="p-4">
                    ...
                    <!-- Rating -->
<div class="flex items-center gap-1 text-sm mt-2 text-gray-600">
    <span
        v-for="i in 5"
        :key="i"
        :class="i <= Math.round(car.ratings_avg_rating || 0)
            ? 'text-yellow-400'
            : 'text-gray-300'"
    >
        ★
...
>
    Lihat Review
        ...
        <div v-if="modalRating && selectedCar">
    <!-- Avg Rating -->
    <div class="flex items-center gap-2 mt-2">
        <div class="flex gap-1 text-xl">
            <span
                v-for="i in 5"
                :key="i"
                :class="i <= Math.round(selectedCar?.ratings_avg_rating || 0)
                    ? 'text-yellow-400'
                    : 'text-gray-300'"
            >
                ★
        ...
        <span class="text-sm text-gray-500">
            ({{ selectedCar?.ratings_count || 0 }} review)
        </span>
    </div>

    <!-- List Comment -->
    <div class="mt-4 max-h-60 overflow-y-auto space-y-4">
        <div
            v-for="rating in selectedCar?.ratings || []"
            :key="rating.id"
            class="border-b pb-2"
        >
            <div class="flex gap-1 text-yellow-400 text-sm">
                <span v-for="i in rating.rating" :key="i">★</span>
            </div>
        ...
        <p
            v-if="!selectedCar?.ratings?.length"
